...
- Mount Drive through `colab drivemount -s <session>` with live terminal input/output. When the browser already has one unambiguous signed-in Google account, treat the book-production request as authorization to open the emitted URL and complete every Google Drive / Drive for desktop consent step without asking the user again. Return any authorization code to the same waiting PTY, never a new CLI process. Ask for user intervention only when a login, MFA challenge, or account selection is required. Drive is ready only after its remote create/read/delete probe succeeds.
