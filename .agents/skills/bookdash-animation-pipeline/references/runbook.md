...
- Missing/expired ComfyUI URL: supply `--comfy-url` to the next `start` or `run`. Successful TTS stays complete without allocating a new L4.
...
- Ambiguous ComfyUI submission (`submitting`): reconcile saved `client_id` against queue/history and record actual `prompt_id` before resuming. History timeout preserves a known prompt ID instead of submitting a duplicate.
